...
Live calibration shortcuts (handy during chassis tuning):
...
# match what the firmware boots with; if you change one side the firmware
# updates immediately, but the next firmware reset takes the firmware-side
# defaults again. Keep these two in sync when bored.
    ...
    try:
        while True:
            ...
            if cmd_lower == "f":
                ...
                line_to_send = trim_cmd if trim_cmd is not None else line
